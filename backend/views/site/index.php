<?php

/* @var $this yii\web\View */

$this->title = 'My Yii Application';
?>
<div class="site-index" id="admin-catalog"">

    <div class="container">
        <div class="row">
            <div class="col-xs-4">
                <div class="menu-grid">
                    <div>
                        <a href="http://youoffer.com/backend/web/catalog">Категории</a>
                    </div>
                    <div>
                        <a href="http://youoffer.com/backend/web/products">Товары</a>
                    </div>
                    <div>
                        <a href="http://youoffer.com/backend/web/currency">Валюты</a>
                    </div>
                    <div>
                        <a href="http://youoffer.com/backend/web/brands">Брэнды</a>
                    </div>
                    <div>
                        <a href="http://youoffer.com/backend/web/merch">Поставщики</a>
                    </div>
                    <div>
                        <a href="http://youoffer.com/backend/web/users">Пользователи</a>
                    </div>
                    <div>
                        <a href="http://youoffer.com/backend/web/parsers">Загрузка товаров xml/yml</a>
                    </div>
                    <div>
                        <a href="http://youoffer.com/backend/web/settings">Настройки</a>
                    </div>
                </div>
            </div>
            <div class="col-xs-8">

            </div>
        </div>
    </div>
</div>
